...
prepare = function()
{
    ...
    $.get("http://www.yamaha.com.cn/support/personal/maintenance/audio-visual/data.txt",
        null,
        function(data,status)
        {
            if(status != "success")
            {
                alert("Error occured when get data from http://www.yamaha.com.cn/support/personal/maintenance/audio-visual/data.txt!");
                return;
            }
    ...
    $.get("http://www.yamaha.com.cn/support/personal/maintenance/proaudio/data.txt",
        null,
        function(data,status)
        {
            if(status != "success")
            {
                alert("Error occured when get data from http://www.yamaha.com.cn/support/personal/maintenance/proaudio/data.txt!");
                return;
            }
    ...
    $.get("http://www.yamaha.com.cn/support/personal/maintenance/music-instruments/data.txt",
        null,
        function(data,status)
        {
            if(status != "success")
            {
                alert("Error occured when get data from http://www.yamaha.com.cn/support/personal/maintenance/music-instruments/data.txt!");
                return;
            }
...
var parseData = function()
{
    ...
    $("#keyword_search .search").click(function()
    {
        ...
        var strInfo = "你搜索的" + strKWCopy + strProduct + "找到'" + resultList.length +"'家维修站"
    ...
    $("#district_search .search").click(function()
    {
        ...
        if(pName != null || cName != null || proName != null)
        {
            ...
            var strInfo = "你搜索的" + strDistrct + strProduct + "找到'" + resultList.length +"'家维修站"
    ...
    function showResult(dealers,$container)
    {
        ...
        $.each(dealers,function(index,dealer)
        {
            ...
            if(dealer.contact != null && dealer.contact.length > 0)
            {
                $(".contact span",$resultView).text(dealer.contact);
            }else{
                $(".contact",$resultView).hide();
            }
